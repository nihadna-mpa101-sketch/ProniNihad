...
using ProniaWebNihad.ViewModels.ProductViewModels;
...
namespace ProniaWebNihad.Areas.Admin.Controllers
{
    ...
    public class ProductController(AppDbContext _context, IWebHostEnvironment _environment) : Controller
    {
        ...
        public async Task<IActionResult> Create(ProductCreateVM vm)
        {
            ...
            {
                ModelState.AddModelError("RatingCount", "Rating0-5 arasi olmalidi!");
                return View(vm);
            }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            string hoverImagePath = Path.Combine(folderUrl, product.HoverImagePath);
            string mainImagePath = Path.Combine(folderUrl, product.MainImagePath);

            ExtensionMethods.DeleteFile(hoverImagePath);
            ExtensionMethods.DeleteFile(mainImagePath);
            foreach (var image in product.ProductImages)
            {
                string ImagePath = Path.Combine(folderUrl, image.ImagePath);
